...
import { formatNumber, getUserIdFromLocalStorage, getUserNameFromLocalStorage } from "../../components/common/utils";
...
export default function PaymentManagement() {
    ...
    const [currentPaymentTeacher, setCurrentPaymentTeacher] = useState<TeacherSalary | null>(null);
    ...
    const handlePaySalary = async (id: string, salary: number, month: number, year: number, teacher: TeacherSalary) => {
        const response = await axios.post(
    ...
    const paymentComplete = async () => {
        await axios.post(`${import.meta.env.VITE_API_URL}/admin/salary-payment-complete`, {
            ...
            <div className="space-y-6">
                ...
                <ComponentCard title="Danh sách thanh toán lương">
                    <Table>
                        <TableHeader >
                            <TableRow >
                                <TableCell className="text-center text-gray-500 dark:text-gray-400">Tên giảng viên</TableCell>
                                <TableCell className="text-center text-gray-500 dark:text-gray-400">Lương</TableCell>
                                <TableCell className="text-center text-gray-500 dark:text-gray-400">Trạng thái thanh toán</TableCell>
                                <TableCell className="text-center text-gray-500 dark:text-gray-400">Tháng</TableCell>
                                <TableCell className="text-center text-gray-500 dark:text-gray-400">Hành động</TableCell>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {teachers.length === 0 ? (
                                ...
                                teachers.map((teacher) => (
                                    <TableRow key={teacher.id} >
                                        <TableCell className="flex items-center gap-2 text-center text-gray-500 dark:text-gray-400 py-1">  <img
                                            src={teacher.teacherId?.avatar}
                                            alt={teacher.teacherId?.fullName}
                                            className="w-10 h-10 rounded-full p-1"
                                        />{teacher.teacherId?.fullName}</TableCell>
                                        <TableCell className="text-center text-gray-500 dark:text-gray-400">{formatNumber(teacher.salary)}</TableCell>
                ...
                <div className="fixed inset-0 bg-black/10 dark:bg-black/70 flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Quét mã QR để thanh toán</h3>
                            <button 
                        ...
                        <Button variant="primary" className="w-full h-10" onClick={() => paymentComplete()}>
                            Hoàn thành
                        </Button>
                    </div>
                </div>
