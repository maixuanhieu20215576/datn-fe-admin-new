...
export default function ChatApp() {
  ...
  const loadMessageHistory = async ({ opponentId }: { opponentId: string }) => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/user/load-message-history`,
        {
          userId: userId,
  ...
  const renderMessageContent = (content: string, senderId: string) => {
    if (isImageFile(content)) {
      return <ResponsiveImage src={content} className="w-full h-64 rounded" />;
      ...
      return (
        <a
          ...
          className={`${
            senderId === userId ? "text-white" : "text-gray-900 dark:text-white"
        ...
        <div className="flex-1 flex flex-col">
                  ...
                  >
                    ...
                    {msg.senderId && (
                      <div
                        className={`${
                          msg.senderId === userId
                            ? "text-right text-gray-100 text-sm"
                            : "text-right text-gray-500 dark:text-white text-sm"
                        }`}
                      >
          ...
          <div className="flex p-4 border-t border-gray-200 dark:border-gray-700 items-center gap-2 bg-gray-100 dark:bg-gray-800">
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => setIsFileInput(!isFileInput)}
            >
